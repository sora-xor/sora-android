...
On this resource tree, Android XML parsing and AAPT resource processing passed. Local `:app:lintProductionRelease` was run with only the protected signing and production Google OAuth validation tasks excluded. It reports 183 `MissingTranslation` errors, 1,021 warnings, and one hint, with no other error class. The two requested keys no longer appear in the lint report; missing locale/key pairs fell from 3,721 to 3,679. The JSON handoff matches that final report. Full protected Release qualification and review of the remaining translations are still required.
