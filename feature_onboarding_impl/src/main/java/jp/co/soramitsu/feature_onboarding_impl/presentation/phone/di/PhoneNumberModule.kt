...
import jp.co.soramitsu.feature_onboarding_impl.presentation.phone.PhoneNumberViewModel
...
class PhoneNumberModule {
    ...
    fun provideViewModel(interactor: OnboardingInteractor, router: OnboardingRouter, progress: WithProgress): ViewModel {
        return PhoneNumberViewModel(interactor, router, progress)
    }

    @Provides
    fun provideViewModelCreator(fragment: Fragment, viewModelFactory: ViewModelProvider.Factory): PhoneNumberViewModel {
        return ViewModelProviders.of(fragment, viewModelFactory).get(PhoneNumberViewModel::class.java)
    }
