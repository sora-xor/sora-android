...
class ReceiveViewModel(
    ...
    private fun generateMessage(amount: String, accountId: String): String {
        val message = if (amount.isEmpty()) {
            resourceManager.getString(R.string.wallet_qr_share_message_empty_template).format(resourceManager.getString(R.string.common_xor))
        } else {
            resourceManager.getString(R.string.wallet_qr_share_message_template).format(amount, resourceManager.getString(R.string.common_xor))
        }
        return message + "\n$accountId"
    }
}