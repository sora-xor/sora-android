...
class ClaimWorker(private val appContext: Context, workerParams: WorkerParameters) : Worker(appContext, workerParams) {
    ...
    @Inject lateinit var walletInteractor: WalletInteractor
    ...
    override fun doWork(): Result {
        ...
        val notification = NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            ...
            .setLargeIcon((ContextCompat.getDrawable(appContext, R.mipmap.ic_launcher) as BitmapDrawable).bitmap)
        ...
        val result = walletInteractor.migrate().doOnError { FirebaseCrashlytics.getInstance().recordException(it) }.onErrorReturnItem(false).blockingGet()
        walletInteractor.saveMigrationStatus(if (result) MigrationStatus.SUCCESS else MigrationStatus.FAILED).blockingAwait()
