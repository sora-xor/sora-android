...
class SendIntegrationTest {
    ...
    fun `check prefix byte`() {
        ...
        val prefix =
            (valueConstant?.type?.fromByteArrayOrNull(runtime, valueConstant.value) as? BigInteger)?.toInt()
                ?: 69
        assertEquals(prefix, 42)
    }

    @Test
    fun `storage assets_assetInfos`() {
        val runtime = TestRuntimeProvider.buildRuntime("sora2")
        ...
        val stotoac = runtime.metadata.module("Tokens").storage("Accounts").storageKey(runtime, address, toke)
        val stora = runtime.metadata.module("Assets").storage("AssetInfos").type.value
        val r = socketService.singleRequest(
            RuntimeRequest(
                "state_getStorage",
                listOf(
                    stotoac
...
//        val sss = ((qwe as? List<*>)?.get(1) as? ByteArray)?.toString(Charsets.UTF_8)
        ...
        assertTrue(free != null && free == BigInteger.ONE)
    }
}
