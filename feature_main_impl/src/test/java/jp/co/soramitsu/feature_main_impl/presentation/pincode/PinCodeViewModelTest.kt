...
class PinCodeViewModelTest {
    ...
    @Mock lateinit var interactor: PinCodeInteractor
    ...
    @Before fun setUp() {
        pinCodeViewModel = PinCodeViewModel(interactor, mainRouter, progress, maxProgress)
    }

    @Test fun `start auth with CREATE_PIN_CODE action`() {
    ...
    @Test fun `start auth with OPEN_PASSPHRASE action`() {
    ...
    @Test fun `pinCodeNumberClicked() changes progress and deleteBtn visibility`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
    ...
    @Test fun `pinCodeDeleteClicked() changes progress and deleteBtn visibility`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
        pinCodeViewModel.pinCodeNumberClicked("1")
    ...
    @Test fun `pin code overclicks`() {
        ...
        verifyNoMoreInteractions(interactor, mainRouter)
        verifyZeroInteractions(progress)
    }

    @Test fun `pin code entered once while creating`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
        pinCodeViewModel.pinCodeNumberClicked("1")
        pinCodeViewModel.pinCodeNumberClicked("2")
        pinCodeViewModel.pinCodeNumberClicked("3")
        ...
        assertEquals(0, pinCodeViewModel.pinCodeProgressLiveData.value!!)
        assertEquals(R.string.pincode_confirm_your_pin_code, pinCodeViewModel.toolbarTitleResLiveData.value)
        assertTrue(pinCodeViewModel.backButtonVisibilityLiveData.value!!)
    }

    @Test fun `pin code entered correctly second time while creating`() {
        ...
        assertNotNull(pinCodeViewModel.checkInviteLiveData.value)
    ...
    @Test fun `pin code entered wrong second time while creating`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
        pinCodeViewModel.pinCodeNumberClicked("1")
        pinCodeViewModel.pinCodeNumberClicked("2")
        pinCodeViewModel.pinCodeNumberClicked("3")
    ...
    @Test fun `pin code entered correct with OPEN_PASSPHRASE action`() {
        pinCodeViewModel.startAuth(PinCodeAction.OPEN_PASSPHRASE)
    ...
    @Test fun `back pressed closing the app on CREATE_PIN_CODE action`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
    ...
    @Test fun `back pressed closing the app on TIMEOUT_CHECK action`() {
        given(interactor.isCodeSet()).willReturn(Single.just(true))
    ...
    @Test fun `back pressed hiding pin code view on OPEN_PASSPHRASE action`() {
        pinCodeViewModel.startAuth(PinCodeAction.OPEN_PASSPHRASE)

        pinCodeViewModel.backPressed()
    }

    @Test fun `back pressed leads to reset pin view on CREATE_PIN_CODE action`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
    ...
    @Test fun `onResume() starts fingerprint scanner on OPEN_PASSPHRASE action`() {
        pinCodeViewModel.startAuth(PinCodeAction.CREATE_PIN_CODE)
    ...
    @Test fun `onResume() starts fingerprint scanner on TIMEOUT_CHECK action`() {
        given(interactor.isCodeSet()).willReturn(Single.just(true))
    ...
    @Test fun `fingerprint scanner success leads to check user fragment on TIMEOUT_CHECK action`() {
        given(interactor.isCodeSet()).willReturn(Single.just(true))
    ...
    @Test fun `onAuthenticationError() set fingerPrintErrorLiveData value`() {
        val message = "test message"
