...
import jp.co.soramitsu.feature_multiaccount_impl.databinding.ViewMnemonicContainerBinding
...
class MnemonicContainerView @JvmOverloads constructor(
    ...
    private val viewBinding =
        ViewMnemonicContainerBinding.inflate(LayoutInflater.from(context), this)
    ...
    fun populateWord(mnemonicWordView: MnemonicWordView) {
        ...
        if (elements.isEmpty()) {
            ...
            val lastChildRight =
                (lastView.layoutParams as LayoutParams).leftMargin + lastView.measuredWidth + wordMargin
            ...
            if (freeSpace > wordSpace) {
                ...
                val lastViewBottom =
                    (lastView.layoutParams as LayoutParams).topMargin + lastView.measuredHeight + wordMargin
                (mnemonicWordView.layoutParams as LayoutParams).setMargins(
                    wordMargin,
        ...
        for (line in 1..maxLines) {
            ...
            for (i in previousLineElementIndex until nextLineElementIndex) {
                ...
                val shouldStartNewLine =
                    previousElement?.let { it.line < currentElement.line } ?: true
                if (shouldStartNewLine) {
                    ...
                    val viewRight =
                        previousElement.wordView.right + wordMargin + wordMargin + currentElement.wordView.width
